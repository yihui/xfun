...
- Fixed `taml_load()` failing on YAML with non-uniform indentation (e.g., child items indented by a different number of spaces than the minimum unit). The `indent_level()` function now uses a stack-based algorithm that correctly handles irregular indentation.
...
- Added an argument `sync` to `submit_cran()` to prevent package authors from forgetting to pull from remote before submitting a package.
