...
3.  **MANDATORY: Run `R CMD check` before pushing**: Run a comprehensive
    `R CMD check` once before you are ready to push (or when explicitly told
    to). Most of the time, just run tests (`Rscript tests/*.R`) instead of a
    full check since the latter is time-consuming.
4.  **MANDATORY: Wait for CI to be green** (for Github Copilot only): After
    pushing code, you MUST wait for GitHub Actions CI to complete successfully
    before claiming the task is done. Do not wait more than 5 minutes for any
    single CI job; if it hasn't finished, skip it and continue your work. Fix
    problems in CI as soon as any job has failed instead of waiting for all jobs
    to finish.
...
6.  **Bump version in PRs** (for Github Copilot only): Bump the patch version
    number in DESCRIPTION once per PR (on the first commit or when you first
    make changes), not on every commit to the PR
